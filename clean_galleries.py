...
def clean_galleries(text):
    # Regex to find ANY gallery block that starts with <!-- Before & After Gallery --> and ends with <!-- Booking / Contato -->
    # We replace ALL of them with NOTHING, effectively stripping them.
    # We need to account for both unescaped and escaped forms
    
    t = re.sub(r'<!-- Before & After Gallery -->.*?<!-- Booking / Contato -->', '<!-- Booking / Contato -->', text, flags=re.DOTALL)
    
    # Also look for any standalone gallery sections just in case they don't end in the comment
    t = re.sub(r'<!-- Before & After Gallery -->\s*<section[^>]*id="gallery".*?</section>', '', t, flags=re.DOTALL)
    t = re.sub(r'<!-- Before & After Gallery -->\s*<section[^>]*id=\\"gallery\\".*?</section>', '', t, flags=re.DOTALL)

    # Now, find the first occurrence of <!-- Booking / Contato --> and prepend the correct gallery
    if '<!-- Booking / Contato -->' in t:
        t = t.replace('<!-- Booking / Contato -->', correct_gallery, 1)
        
    return t

def clean_galleries_escaped(text):
    correct_gallery_escaped = correct_gallery.replace('\\n', '\\\\n').replace('\"', '\\\\\"')
    # Regex for escaped text
    t = re.sub(r'<!-- Before & After Gallery -->.*?<!-- Booking / Contato -->', '<!-- Booking / Contato -->', text, flags=re.DOTALL)
    if '<!-- Booking / Contato -->' in t:
        t = t.replace('<!-- Booking / Contato -->', correct_gallery_escaped, 1)
    return t
...
print("Properly cleaned all duplicate galleries!")
